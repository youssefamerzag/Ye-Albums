...
function App() {
      ...
      songs: [
        {
          ...
          img: donda,
          time: "2:58",
          playSound: NoChildLeftBehind,
        },
        {
          song: "Come to Life",
          img: donda,
          time: "5:10",
          playSound: Cometolife,
        },
  ...
  const itemRefs = useRef(albums[albumnb1].songs.map(() => React.createRef()));
  ...
  function right() {
    ...
    for (let i = 0; i < itemRefs.current.length; i++) {
      itemRefs.current[i].current.className = 'fa fa-play';
    }
  ...
  const handleAudio = (index) => {
    ...
    if (currentaudioindex !== null) {
      ...
      itemRefs.current[currentaudioindex].current.className = 'fa fa-play';
    }
  
    if (currentaudioindex !== index) {
      audioRef.current[index].play();
      itemRefs.current[index].current.className = 'fa fa-pause';
      setcurrentaudioindex(index);
    } else {
      audioRef.current[index].pause();
      audioRef.current[index].currentTime = 0;
      itemRefs.current[index].current.className = 'fa fa-play';
  ...
  return (
    <div className="App">
      ...
      <div id="albums">
        <button id="abutton" onClick={left}>
          <i
        ...
        <button id="abutton" onClick={right}>
          <i
        ...
        <ul>
          {albums[albumnb1].songs.map((albumsongs, index) => (
            <li key={index}>
              ...
              <button className="playbutton" onClick={() => handleAudio(index)}>
                <i key={index}  ref={itemRefs.current[index]} className='fa fa-play' style={{ fontSize: "20px" }}></i>
              </button>
              <p>{albumsongs.song}</p>
              <audio
                ref={(audio) => audioRef.current.push(audio)}
                src={albumsongs.playSound}
              />
            </li>
          ))}
        </ul>
      ...
      <li id="youssef">
        Copyright ©
        <a href="https://github.com/youssefamerzag">Youssef Amerzag</a>
      </li>
    </div>
  );
}
